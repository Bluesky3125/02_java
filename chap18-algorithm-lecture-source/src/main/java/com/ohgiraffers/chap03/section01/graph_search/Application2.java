...
public class Application2 {
    ...
    static class Node {         // 내부(inner) 클래스
        int x;
        int y;
    ...
    /* 설명. 배추가 상하좌우를 보고 방문하지 않고 유효한 범위 안에 또다른 배추가 심어져 있는지 확인(방문배열 체크) */
    private static void bfs(int x, int y) {
        ...
        while (!q.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                /* 설명. 지금 보는 방향이 좌표로써 존재하면서 방문한 적이 없고 배추가 심어져 있다면 (방문 체크 + queue에 추가) */
