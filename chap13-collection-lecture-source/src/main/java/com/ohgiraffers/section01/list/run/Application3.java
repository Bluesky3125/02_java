...
//        List<String> list = new LinkedList<>();
        ...
        System.out.println();
        /* 설명. 2. for문 활용하기(조건이 있는 경우) */
        for (int i = 1; i < list.size(); i++) {
            System.out.println(list.get(i));
        }

        System.out.println();
        /* 설명. 3. for-each문 활용하기(모든 요소를 순회하는 경우) */
        for (String str : list) {
        ...
        System.out.println();
        /* 설명. 1번 인덱스(두 번째)의 과일 수정(ArrayList 우위) */
        list.set(1, "pineapple");
        System.out.println("2번째 요소 수정 후: " + list);

        /* 설명. list가 관리하는 요소 제거(LinkedList 우위) */
        /* 설명. 3번째 요소 제거 */
        list.remove(2);
        ...
        list = null;    // null을 대입하면 이루 list를 활용하는 곳에서 NullPointerException이 발생할 수 있다.
